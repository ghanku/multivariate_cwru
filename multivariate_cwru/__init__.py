...
class CWRU:
    ...
    def __init__(self, experiment, length, trainsplit, seed, sens, *rpm, normal_condition=True):
        ...
        fmeta = os.path.join(cur_path, "datafiles2.txt")
                ...
                for idx, info in enumerate(infos):
                    ...
                    if self.sens == 2:
                        ...
                        time_series = np.hstack(
                            (mat_dict[key1], mat_dict[key2]))
                    if self.sens == 3:
                        key1, key2, key3 = keyfilter(
                            mat_dict.keys(), self.sens)
                        time_series = np.hstack(
                            (mat_dict[key1], mat_dict[key2], mat_dict[key3]))
                    if self.sens == 1:
                    ...
                    idx_last = -(time_series.shape[0] % l)
                    if idx_last < 0:
                        clips = time_series[:idx_last].reshape(
                            -1, l, self.sens)
                    else:
                        clips = time_series[idx_last:].reshape(
                            -1, l, self.sens)
                    ...
                    if (first == True):
                        self.x_train = np.zeros((0, l, self.sens))
                        self.x_test = np.zeros((0, l, self.sens))
                        self.y_train = []
                        self.y_test = []
                        first = False
                    ...
                    print(clips[:n_split].shape)
                    self.x_train = np.concatenate(
                        (self.x_train, clips[:n_split]), axis=1)
                    self.x_test = np.concatenate(
                        (self.x_test, clips[n_split:]), axis=1)
